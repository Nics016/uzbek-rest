...
<svg
  ...
  .st1{fill:#F8AC39;}
     ...
     transform="matrix(0.40533209,0,0,0.28902257,-0.8707061,-0.56804606)"
     id="g3918"
     style="opacity:0.85"><ellipse
       ...
       rx="236.16731"
       ry="138.14752"
       style="opacity:0.85;fill:#061929;stroke-width:0.84841007" /></g><path
     class="st0"
     ...
     id="g68"><path
      ...
      <div class="text">
        <span>ресторан изысканной</span>
        <span>восточной кухни</span>
      </div>
      <div class="container">
        <div class="misc">
...
<?php
$menuarr = array();
if ($result = $sql->query("SELECT * FROM Content where Parent = $menupath[0] order by Sort")) {
  while ($row = $result->fetch_assoc()) {
    $res = $sql->query("SELECT * FROM Content where Parent = ".$row['Id']." order by Sort");
    while ($rw = $res->fetch_assoc()) {
      $pth = getPath($row['Id']);
      $tmparray = array($pth[0],$rw['urlName'],$rw['MenuName']);
      $menuarr[] = $tmparray;
    }
  }
} 
//print_r($menuarr);
          ?>
            <ul class="jump">
              <?=genMenuItem($menuarr[0])?>
              <?=genMenuItem($menuarr[1])?>
            </ul>
            <ul class="jump">
              <?=genMenuItem($menuarr[2])?>
            </ul>
            <ul class="jump">
              <?=genMenuItem($menuarr[3])?>
            </ul>
            <ul class="jump">
              <?=genMenuItem($menuarr[4])?>
            </ul>
          </div>
          <div class="right">
            <ul class="jump">
              <?=genMenuItem($menuarr[5])?>
            </ul>
            <ul class="jump">
              <?=genMenuItem($menuarr[6])?>
            </ul>
            <ul class="jump">
              <?=genMenuItem($menuarr[7])?>
            </ul>
            <ul class="jump">
              <?=genMenuItem($menuarr[8])?>
            </ul>
          </div>
        </div>
      </div>
